    ...
    <div className="accounting-page">
      ...
      <div className="accounting-header">
        <div className="header-left">
          ...
          <div>
            <h1 className="accounting-title">Accounts & Financial Ledger</h1>
            <p className="accounting-subtitle">
              Monitor institutional revenues, operational expenditures, and campus cashflow.
            </p>
          </div>
        </div>
        <button className="btn-log-txn" onClick={() => setShowModal(true)}>
          + Record Voucher
        </button>
      </div>
      ...
      {summary && (
        <div className="accounting-summary-grid">
          <div className="accounting-card">
            <span className="card-label">Total Revenue (Inflow)</span>
            <h3 className="card-value income">${summary.totalIncome.toLocaleString()}</h3>
            <span className="card-subtext">Fees, grants, and campus receipts</span>
          </div>

          <div className="accounting-card">
            <span className="card-label">Total Expenses (Outflow)</span>
            <h3 className="card-value expense">${summary.totalExpense.toLocaleString()}</h3>
            <span className="card-subtext">Payroll, utilities, and procurement</span>
          </div>

          <div className="accounting-card">
            <span className="card-label">Operating Balance</span>
            <h3 className={`card-value ${summary.netBalance >= 0 ? "income" : "expense"}`}>
              ${summary.netBalance.toLocaleString()}
            </h3>
            <span className="card-subtext">Net reserves across institutional accounts</span>
          </div>
        </div>
      )}
      ...
      <div className="accounting-table-card">
        <table className="accounting-table">
          ...
          <tbody>
            {loading ? (
              <tr>
                <td colSpan="7" className="table-empty-msg">Loading financial records...</td>
              </tr>
            ) : filteredTransactions.length === 0 ? (
              <tr>
                <td colSpan="7" className="table-empty-msg">
                  No financial transactions found. Click "+ Record Voucher" to log one.
                </td>
              </tr>
            ) : (
              filteredTransactions.map((t) => (
                <tr key={t._id}>
                  <td className="cell-ref">{t.referenceNo}</td>
                  <td>
                    <div className="cell-title">{t.title}</div>
                    {t.remarks && <div className="cell-sub">{t.remarks}</div>}
                  </td>
                  <td>
                    <span className="category-badge">{t.category}</span>
                  </td>
                  <td className="cell-mode">{t.paymentMethod}</td>
                  <td className="cell-date">{new Date(t.date).toLocaleDateString()}</td>
                  <td>
                    <span className={`type-badge ${t.type.toLowerCase()}`}>
                      {t.type}
                    </span>
                  </td>
                  <td className={`cell-amount ${t.type.toLowerCase()}`}>
            ...
            <form onSubmit={handleSubmit} className="accounting-form">
              ...
              <div className="form-row">
                ...
                <div className="form-item">
                  ...
                  >
                    <option value="Staff Salary">Staff / Faculty Salary</option>
